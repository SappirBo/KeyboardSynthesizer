#include "audio_player.hpp"
    ...
    void AudioPlayer::PortAudioPlayer::open_stream(paData &data)
    {
        PaError err = Pa_OpenStream(
              ...
              AudioSettings::FRAME_BUFFER_SIZE,
    ...
    void AudioPlayer::PortAudioPlayer::stop_stream()
    {
        ...
        if( err != paNoError )
        {
            std::cout << "PortAudio StopStream error: " << Pa_GetErrorText( err ) << std::endl;
            exit(0);
        }
    }
    ...
    void AudioPlayer::PortAudioPlayer::start_stream()
    {
        ...
        if( err != paNoError )
        {
            std::cout << "PortAudio StartStream error: " << Pa_GetErrorText( err ) << std::endl;
            exit(0);
        }
    }
