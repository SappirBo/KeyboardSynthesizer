...
void Oscillator::set_sine_wave_table()
{
    ...
    {
        m_paData.wave_table[i] = (float) sin( ((float)i / (float)AudioSettings::TABLE_SIZE) * 2.0 * AudioSettings::PI );
    }
}
void Oscillator::set_square_wave_table()
{
    for(int i = 0; i < AudioSettings::TABLE_SIZE; i++)
    {
        double t = static_cast<double>(i) / static_cast<double>(AudioSettings::TABLE_SIZE);
        if(t < 0.5)
